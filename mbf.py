	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				tampil('\rm[!]Masukan Opsinya bang...')
				continue
		else:
			if len(a) == 0:
				tampil('\rm[!]Masukan dengan benar bang')
				continue
			else:
				break
	return a
...
def login():
	global log
	us = inputD('[?]Email/HP abang')
	pa = inputD('[?]Kata Sandi abang')
	tampil('\rh[*]Sedang Login sambil ngopi bang enak....')
